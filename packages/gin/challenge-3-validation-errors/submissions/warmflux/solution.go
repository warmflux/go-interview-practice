...
func validateProduct(product *Product) []ValidationError {
	...
	if !isValidSKU(product.SKU) {
		errors = append(errors, ValidationError{
			Field:   "sku",
			Message: "SKU must fit (3 letters, 3 numbers, 3 letters)",
		})
	}
...
func createProduct(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&product); err != nil {
		...
		if errors.As(err, &ve) {
			for _, fe := range ve {
				error = append(error, ValidationError{
					Field: fe.Field(),
					Tag:   fe.Tag(),
					Value: fe.Value(),
					Param: fe.Param(),
		...
		c.JSON(400, APIResponse{
			...
			Errors:  error, // TODO: Convert gin validation errors
		})
		return
	}
...
func validateSKUEndpoint(c *gin.Context) {
	...
	if !isValidSKU(request.SKU) {
		c.JSON(200, APIResponse{
			Success: false,
			Message: "SKU is not fit",
		})
		return
	}

	for _, p := range products {
		if request.SKU == p.SKU {
			c.JSON(200, APIResponse{
				Success: false,
				Message: "SKU is exists",
			})
			return
		}
	}
